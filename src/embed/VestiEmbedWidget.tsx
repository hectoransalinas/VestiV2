...
const FitOverlay: React.FC<OverlayProps> = ({
  ...
  // Calzado: el overlay debe ser coherente con el talle evaluado/recomendado (sizeLabel)
  // y con el resultado del motor (fit.lengths). Si no, aparece un talle distinto al de
  // la tarjeta (ej: tarjeta 40, overlay 41).
  const shoeFit = (() => {
    if (!isShoesView) {
      return shoeFitFromFootLength(footLength);
    }

    const shoeZone = (fit?.lengths ?? []).find(
      (z) => z.zone === "pieLargo" || z.zone === "largoPie" || z.zone === "pie",
    );

    // Fallback si el motor no devuelve zona de calzado.
    if (!shoeZone) {
      const base = shoeFitFromFootLength(footLength);
      return {
        label: garmentSizeLabel ? `${garmentSizeLabel} (${base.label.replace(/^[0-9]+\s*\(|\)$/g, "")})` : base.label,
        statusKey: base.statusKey,
      };
    }

    const raw = String(shoeZone.status ?? "Perfecto");
    const statusKey: "Perfecto" | "Ajustado" | "Holgado" =
      raw === "Perfecto"
        ? "Perfecto"
        : raw === "Corto" || raw === "Ajustado" || raw === "Justo"
        ? "Ajustado"
        : "Holgado";
    ...
    const labelCore = garmentSizeLabel ?? mapFootToEuSize(footLength) ?? "—";
    return {
      label: `${labelCore} (${statusText})`,
      statusKey,
    };
  })();
...
export const VestiEmbedWidget: React.FC<VestiEmbedProps> = ({
  ...
  // En Shopify abrimos el widget como "guía de talles" (mode=sizeguide).
  // Solo en ese modo ajustamos proporciones para que entre todo en un vistazo.
  const isSizeGuideMode = useMemo(() => {
    try {
      const qs = new URLSearchParams(window.location.search);
      return qs.get("mode") === "sizeguide";
    } catch (_err) {
      return false;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          width: "100%",
          aspectRatio: isSizeGuideMode ? "16 / 10" : "9 / 16",
          ...(isSizeGuideMode ? { height: "min(560px, 62vh)" } : null),
          borderRadius: 16,
          overflow: "hidden",
          background: "#f9fafb",
          marginBottom: 8,
        ...
        {avatarUrl ? (
          <>
            ...
            <FitOverlay
              ...
              garmentSizeLabel={(prenda as any)?.sizeLabel}
            />
          </>
        ) : (
          <>
