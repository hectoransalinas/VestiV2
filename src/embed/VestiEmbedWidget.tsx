...
export const VestiEmbedWidget: React.FC<VestiEmbedProps> = ({
  ...
  // En modo sizeguide, el perfil viene del panel izquierdo (acordeón). Si cambia, debemos
  // sincronizarlo aquí para que overlays + recomendación se actualicen.
  useEffect(() => {
    if (!isSizeGuideMode) return;
    if (!perfilInicial) return;
    setUser((prev) => ({ ...prev, ...perfilInicial }));
    // Si el perfil trae pieLargo, lo reflejamos también en footLength (shoes).
    const anyPerfil: any = perfilInicial as any;
    if (typeof anyPerfil.pieLargo === "number" && Number.isFinite(anyPerfil.pieLargo)) {
      setFootLength(anyPerfil.pieLargo);
    }
  ...
  return (
    ...
    >
      ...
      >
        {avatarUrl ? (
          <>
            <div style={{ width: "100%", height: "100%", transform: isSizeGuideMode ? "scale(0.88)" : "none", transformOrigin: "center top" }}>
          ...
          <>
            <div style={{ width: "100%", height: "100%", transform: isSizeGuideMode ? "scale(0.9)" : "none", transformOrigin: "center top" }}>
            <iframe
              ref={iframeRef}
